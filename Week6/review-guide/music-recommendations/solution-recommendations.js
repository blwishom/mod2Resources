...
class MusicRecommendation {
  ...
  getFriendIDs(userID, degree = 1) {
    ...
    while (queue.length > 0) {
      ...
      if (!visited.has(currentID)) {
        ...
        if (currentDegree === degree) {
          friendIDs.push(currentID);
        }
        let friends = this.users[currentID].friends;
        for (let i = 0; i < friends.length; i++) {
          let pathCopy = [...path];
          pathCopy.push(friends[i]);
          queue.push(pathCopy);
        }
      }
    }
  ...
  genresLiked(userID) {
    ...
    for (let i = 0; i < user.genres.length; i++) {
      genres.push(this.genres[user.genres[i]]);
    }
  ...
  similarityScore(userOneID, userTwoID) {
    const set1 = new Set(this.users[userOneID].genres);
    const set2 = new Set(this.users[userTwoID].genres);

    let commonGenres = new Set([...set1].filter((x) => set2.has(x)));
    let totalGenres = new Set([...set1, ...set2]);

    return commonGenres.size / totalGenres.size;
  }
  ...
  recommendFriends(userID, degrees = 2) {
    ...
    for (let degree = 2; degree <= degrees; degree++) {
      let newRecs = this.getFriendIDs(userID, degree);

      newRecs.sort((a, b) => {
        let scoreA = this.similarityScore(userID, a);
        let scoreB = this.similarityScore(userID, b);
        if (scoreA === scoreB) {
          return a - b;
        } else {
      ...
      friendRecs = friendRecs.concat(newRecs);
    }
...
function tryLocalTests() {
  ...
  for (let i = 0; i < friendRecs.length; i++) {
    console.log(
      friendRecs[i] + ": " + smallRecs.similarityScore(1, friendRecs[i])
    );
  }
  // 3: 0.5
  // 5: 0.2
  ...
  for (let i = 0; i < moreFriendRecs.length; i++) {
    console.log(
      moreFriendRecs[i] + ": " + smallRecs.similarityScore(1, moreFriendRecs[i])
    );
  }
  // 3: 0.5
  // 5: 0.2
